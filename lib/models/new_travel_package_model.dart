...
class NewTravelPackageModel {
  ...
  factory NewTravelPackageModel.fromMap(Map<String, dynamic> data) {
    return NewTravelPackageModel(
      id: data['id'],
      name: data['name'],
      itinerary: data['itinerary'] != null
          ? Map<int, List<String>>.from(
              data['itinerary'].map((key, value) {
                // Convert the key from String to int, and keep the value as List<String>
                return MapEntry(
                    int.tryParse(key) ?? 0, List<String>.from(value));
              }),
            )
          : {}, // Default to an empty map if 'itinerary' is null
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'itinerary': itinerary != null
          ? itinerary.map((key, value) {
              return MapEntry(
                  key.toString(), value); // Convert the int key back to string
            })
          : {}, // Handle null itinerary
      'price': price,
      'duration': duration,
